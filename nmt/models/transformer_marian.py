# coding: utf-8
"""
Marian version of Transformer model for translation
"""
...
class MultiheadAttention(nn.Module):
    def __init__(
        ...
        self.k_linear = nn.Linear(embedding_size, embedding_size, bias=True)
        self.v_linear = nn.Linear(embedding_size, embedding_size, bias=True)
        self.q_linear = nn.Linear(embedding_size, embedding_size, bias=True)
        self.o_linear = nn.Linear(embedding_size, embedding_size, bias=True)
...
def create_positional_encoding_vector(embedding_size: int, max_length: int):
    ...
    s = torch.exp(
        ...
        (math.log(1e4) / (embedding_size // 2 - 1))
    )
    pe[:, :embedding_size // 2] = torch.sin(p * s)
    pe[:, embedding_size // 2:] = torch.cos(p * s)
    return pe
...
def create_positionwise_feedforward(
    ...
    return nn.Sequential(
        nn.Linear(embedding_size, ffn_size), SiLU(), nn.Dropout(dropout),
...
class Model(EncoderDecoder):
    def __init__(
        ...
        self.logit_bias = nn.Parameter(torch.zeros(len(tgt_vocab.itos)))
        if src_vocab == tgt_vocab:
            self.tgt_embed__ = None
            self.o_layer = lambda x: torch.matmul(x, self.src_embed.weight.t()) + self.logit_bias
        else:
            self.tgt_embed__ = nn.Embedding(
                len(tgt_vocab.itos),
                embedding_size,
                padding_idx=src_vocab.pad_index
            )
            self.o_layer = lambda x: torch.matmul(x, self.tgt_embed__.weight.t()) + self.logit_bias
    ...
    def description():
        ...
        Transformer Marian
        ====================
        The marian transformer model based on "Attention is all you need" paper by wasvani et al.
        """

    @property
    def tgt_embed(self):
        if self.tgt_embed__ is not None:
            return self.tgt_embed__
